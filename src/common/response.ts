...
export default function (req: express.Request, res: express.Response, code?: any, data?: any) {
    try {
        ...
        // 핀번호 미설정시
        if (req.method === 'POST' && req.originalUrl.startsWith('/token') && (res.locals.payload && res.locals.payload.pinReset)) {
            response.result.code = CODE.Auth.PinReset.code;
            response.result.message = '핀 번호를 설정해주세요';
        };
